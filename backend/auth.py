...
SUPABASE_JWT_SECRET = os.getenv("SUPABASE_JWT_SECRET", "super-secret-jwt-token-with-at-least-32-characters-long") # fallback or actual
...
def require_super_admin(user_payload: dict = Security(get_current_user)):
    ...
    if email == "ashokvallabhuni28@gmail.com":
        return user_payload

    # Or check via database RPC if user has admin access
