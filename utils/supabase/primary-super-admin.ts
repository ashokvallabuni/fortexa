...
export async function ensurePrimarySuperAdmin(user: User): Promise<{ ok: boolean; error?: string }> {
  try {
    ...
    console.error('Primary Super Admin provisioning failed (non-blocking):', provisioningError);
    return {
      ok: false,
